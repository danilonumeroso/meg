...
from models import MolDQN
from models.Environment import Molecule
...
encoder.load_state_dict(torch.load("ckpt/Encoder.pth"))
encoder.eval()
...
class QEDRewardMolecule(Molecule):
    ...
    def _reward(self):
        ...
        X = one_hot(X, num_classes=50).float()

        E = torch.tensor([[bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()] for bond in molecule.GetBonds()]).t()

        d = Data(x=X, edge_index=E)
        d.batch = torch.zeros(X.shape[0]).long()

        if len(d.edge_index) == 0:
            return 0

        reward, yc  = encoder(d).max(dim=1)
        reward, yc  = reward[0].item(), yc[0].item()

        reward = int(yc == CLASS_TO_EXPLAIN) - reward
